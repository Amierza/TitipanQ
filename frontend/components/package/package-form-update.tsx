...
const PackageFormUpdate = ({
  ...
  useEffect(() => {
    if (packageData?.status && packageData.data && senders.length > 0) {
      ...
      const senderFromData =
        senders.find(
          (s) =>
            s.sender_id?.toLowerCase().trim() ===
            pkg.sender.sender_id?.toLowerCase().trim()
        ) || null;

      setSelectedSender(senderFromData);

      methods.reset({
        sender_id: pkg?.sender.sender_id || '',
        package_tracking_code: pkg.package_tracking_code || '',
        package_quantity: pkg.package_quantity?.toString() || '',
        package_description: pkg.package_description || '',
        package_type: (pkg.package_type as PackageType) || PackageType.Document,
        user_id: pkg.user?.user_id || '',
        locker_id: pkg.locker?.locker_id || '',
        package_status:
          (pkg.package_status as PackageStatus) || PackageStatus.Received,
      });
    }
  }, [packageData, senders, methods]);
