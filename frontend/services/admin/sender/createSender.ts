...
export const createSenderService = async (
  data: z.infer<typeof SenderSchema>
): Promise<SenderResponse | ErrorResponse> => {
  const token = localStorage.getItem('access_token');
  try {
    const response = await axiosAdminConfig.post(
      `${baseUrl}/admin/create-sender`,
      data,
      {
        headers: {
    ...
    return {
      ...
      message:
        ...
        'Terjadi kesalahan saat melakukan pembuatan data sender.',
